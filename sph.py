...
class sph_solver:
    ...
    def viscosityForce(self, ptc_i, ptc_j, r, r_mod):
        ...
        if v_xy < 0:
            ...
            res = -self.m * vmu * v_xy / (r_mod ** 2 + 0.01 * self.dx ** 2) * self.cubicKernelDerivative(r_mod,
                                                                                                         self.dh) * r / r_mod
        return res
    ...
    def updateTimeStep(self):
        ...
        for p_i in self.particle_positions:
            ...
            self.particle_pressure[p_i][0] = self.pUpdate(self.particle_density[p_i][0], self.rho_0, self.gamma,
                                                          self.c_0)
