...
for i in range(-10, 10, 6):
    for j in range(-10, 10, 6):
        ...
        if choose < 0.4:
            spheres[k] = np.array([center[0], center[1], center[2], 0.5, 0.0, r() * r(), r() * r(), r() * r()],
                                  dtype=np.float32)
        elif choose < 0.7:
            spheres[k] = np.array(
                [center[0], center[1], center[2], 0.5, 1.0, 0.5 * (1 + r()), 0.5 * (1 + r()), 0.5 * (1 + r())],
                dtype=np.float32)
        else:
...
def paint():
    for i, j in pixels:
        ...
        for _ in ti.static(range(4)):
            ...
            direction = ti.Vector(lower_left_corner) + uu * ti.Vector(horizontal) + vv * ti.Vector(
                vertical) - ti.Vector(origin) - offset
